...
class Dataset(dataset):
    def __init__(self, train=True):
        ...
        self.USER, self.CARD, self.CARD_IDX, self.ITEM_CAND, self.ITEM_POS = load_gen_data(hp.gen_data_train if train else hp.gen_data_test)

    def __len__(self):
        return len(self.USER)

    def __getitem__(self, i):
        user = np.asarray(self.USER[i])
        card = np.asarray(self.CARD[i])
        card_idx = np.asarray(self.CARD_IDX[i])
        item_cand = np.asarray(self.ITEM_CAND[i])
        item_pos = np.asarray(self.ITEM_POS[i])
        mask_dec = np.zeros((len(card_idx), len(item_cand)), dtype=np.bool)
        for i in range(1, len(card_idx)):
            cur_mask = np.zeros(len(item_cand), dtype=np.bool)
            cur_mask[card_idx[i - 1]] = True
            mask_dec[i, :] = mask_dec[i - 1] | cur_mask

        return user, card, card_idx, item_cand, item_pos, mask_dec


if __name__ == "__main__":
    user, card, card_idx, item_cand, item_pos, mask_dec = Dataset()[0]
    print(mask_dec)
    # print(user)
    # print(card)
    # print(card_idx)
